...
class AsyncGitHubClient(object):
    ...
    def authenticate(self):
        self.auth = {
            'client_id': self.options.github_oauth_key,
            'client_secret': self.options.github_oauth_secret,
            'access_token' : self.options.github_api_token,
        }
    ...
    def fetch(self, url, callback=None, params=None, **kwargs):
        ...
        if not url.startswith(self.options.github_api_url):
            raise ValueError(
    ...
    def github_api_request(self, path, callback=None, **kwargs):
        ...
        """
        url = url_path_join(self.options.github_api_url, path)
